...
def uploaded_file(filename):
    ...
    def absorption_onset(input_array_x, input_array_y, text1, text2, x_direction, y_norm_not=y_norm_not):
        ...
        if x_direction == 'increasing':
            ...
            index_min = np.argmax(slope_test)
        ...
        for i in range(len(slope_test)):
            ...
            for cnt, j in enumerate(near_inflec_x):
                ...
                if cnt == len(near_inflec_x) -1:
                    ...
                    #print(sum(match))
                    #print(1000*slope_test[i])
                    x_big_range = np.amax(array_x) - np.amin(array_x)
                    
                    best_fitting_range.append(sum(match) + -1.5 *1000*slope_test[i]) # might need adjustments for other units
        ...
        # this section appears to make it better at choosing the right line
